...
Once the functions have been identified, we'll go ahead and build out the configuration in our `configuration.toml` file under the `[Writable.Pipeline]` section:
...
The first line of note is `ExecutionOrder = "FilterByDeviceName,TransformToXML,SetOutputData"`. This specifies the order in which to execute your functions. Each function specified here must also be placed in the `[Writeable.Pipeline.Functions]` section. 
